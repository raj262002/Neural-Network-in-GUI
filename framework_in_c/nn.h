...
#define ARRAY_LENS(xs) sizeof(xs)/sizeof(xs[0])
...
typedef struct {
    ...
    size_t cols;
    size_t stride;
    float *es; //pointer to the beginning of data in metrics
} Mat;

#define MAT_AT(m, i, j) (m).es[(i)*(m).stride +(j)]

//malloc dynamic allocate floats 
...
void nn_backprop(NN nn, NN g, Mat ti, Mat to){
    ...
    for(size_t i = 0; i < n; ++i){
        ...
        for (size_t j = 0; j < to.cols; j++){
            MAT_AT(NN_OUTPUT(g), 0, j) = MAT_AT(NN_OUTPUT(nn), 0, j) - MAT_AT(to, i, j);  //using free memory wasted in coping 
        }
